...
ch_bycovg <- ch %>%
  ...
  summarise(percent_tpp = mean(percent_tpp, na.rm = TRUE),
            cov_gt_cutoff = case_when(percent_tpp >= ch_cov_cutoff ~ "include",
                                      percent_tpp < ch_cov_cutoff ~ "exclude")) 

ch_bycovg <- split(ch_bycovg, ch_bycovg$cov_gt_cutoff) %>%
  lapply(FUN = function(x) unique(pull(x, household_id)))

include <- ch_bycovg$include
exclude <- ch_bycovg$exclude
...
ch_chars <- ch %>%
  group_by(household_id) %>%
  summarise(include = (mean(percent_tpp, na.rm = TRUE) >= ch_cov_cutoff),
...
setkey(ch_wevent, include, household_id, msoa, region, n_resid, ch_size, ch_type, 
       ...
       hh_p_dem, hh_dem_gt50, first_event, ever_affected, first_event_which, date)
...
print("Homes in all analysis data:")
dat %>%
  group_by(include) %>%
  summarise(N = n_distinct(household_id))

# Keep only care homes with sufficient coverage
dat <- dat %>%
  filter(include)

print("Homes in final analysis data:")
summary(dat)
